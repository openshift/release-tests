...
def image_consistency_check(ctx, build_number):
    ...
    if not build_number:
        ...
        image_consistency_result = report.get_task_status(LABEL_TASK_IMAGE_CONSISTENCY_TEST)
        if image_consistency_result == TASK_STATUS_PASS:
            logger.info("image consistency check already pass, not need to trigger again")
        elif image_consistency_result == TASK_STATUS_INPROGRESS:   
            logger.info("job[image-consistency-check] already triggered and in progress, not need to trigger again")
        else:
            ...
            nm.sc.post_message(cs.get_slack_channel_from_contact("qe"), "["+cs.release+"] image-consistency-check job: "+build_url)
            report.update_task_status(LABEL_TASK_IMAGE_CONSISTENCY_TEST, TASK_STATUS_INPROGRESS)
    else: 
        logger.info(f"check image consistency check job status according  to job id:{build_number}")
        jh.get_job_status(cs.get_jenkins_server(), "image-consistency-check", build_number)
