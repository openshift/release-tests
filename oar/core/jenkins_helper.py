...
class JenkinsHelper:
    def __init__(self, cs: ConfigStore):
        ...
        self.errata_numbers = ' '.join([str(i)for i in [val for val in self._cs.get_advisories().values()]])
    ...
    def call_stage_job(self): 
        try:
            ...
            server.build_job("Stage-Pipeline",parameters={'VERSION': self.version,'METADATA_AD':self.metadata_ad,'PULL_SPEC':self.pull_spec})
    ...
    def call_image_consistency_job(self): 
        try:
            server = jenkins.Jenkins(self._cs.get_jenkins_server(), username=self._cs.get_jenkins_username(), password=self._cs.get_jenkins_token())
            server.build_job("image-consistency-check",parameters={'VERSION': self.vversion,'ERRATA_NUMBERS':self.errata_numbers,'PAYLOAD_URL':self.pull_spec})
        except jenkins.JenkinsException as ej:   
            raise JenkinsHelperException("call image-consistency-check pipeline job failed") from ej
        last_build_number = server.get_job_info("image-consistency-check")['lastBuild']['number']
        if not last_build_number:
    ...
    def get_job_status(self, url, job_name, build_number):
        ...
        """
        try:
            server = jenkins.Jenkins(url, username=self._cs.get_jenkins_username(), password=self._cs.get_jenkins_token())
            server.assert_job_exists
            try:
                build_info = server.get_build_info(job_name, build_number)
            except jenkins.JenkinsException as je:
                raise JenkinsHelperException(f"job[{job_name}] number [{build_number}] does not exist") from je
            
            if job_name == "signature_check":
                zstream_version = build_info['actions'][0]['parameters'][0]['value'] 
            else:
                pattern = "4.\d{2}.\d{1}|\d{2}}"
                if (build_info['actions'][0]['_class']=='hudson.model.ParametersAction'):
            ...
            if zstream_version.find(self._cs.release)!=-1:
                ...
                if build_info['building']:
                    logger.info(f"{job_name} job: {build_number} status is: In Progress")
                else:
                    status = server.get_build_info(job_name, build_number)['result']
                    logger.info(f"{job_name} job: {build_number} status is: {status}")
            else:
                logger.error(f"{url}job/{job_name}/{build_number} version is not [{self._cs.release}], please provide correct job id")
        except:
            raise JenkinsHelperException("get job{job_name}:{build_number} status failed")
