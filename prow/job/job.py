...
class Jobs:
    ...
    def get_sha(self, url):
        ...
        res = requests.get(url=url, headers=release_tests_github_headers())
    ...
    def push_action(self, url, data):
        ...
        res = requests.put(url=url, json=data,
                           headers=release_tests_github_headers())
        if res.status_code == 200:
            print(res.reason)
        else:
            print(res.status_code, res.reason)
    ...
    def list_jobs(self, component, branch):
        ...
        req = requests.get(url=base_url, headers=openshift_release_github_headers(), timeout=3)
        if req.status_code == 200:
            file_dict = yaml.load(req.text, Loader=yaml.FullLoader)
            file_count = 0
            for file in file_dict:
